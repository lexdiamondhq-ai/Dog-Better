...
export default function NewPost() {
  ...
  const { circleId, seed } = useLocalSearchParams<{ circleId?: string; seed?: string }>();

  const [uri, setUri] = useState<string | null>(null);
  const [caption, setCaption] = useState(typeof seed === 'string' ? seed : '');
  ...
  return (
    <Screen keyboardShouldPersistTaps="handled">
      <ScreenHeader title="Post to your circle" onBack={() => router.back()} large={false} />
      ...
      <Animated.View entering={FadeInUp.delay(40).duration(260)}>
        {uri ? (
          <Animated.View entering={FadeIn} style={[styles.preview, { borderColor: t.border }]}>
            ...
            <Tap onPress={() => setUri(null)} haptic="selection" style={[styles.remove, { backgroundColor: t.scrim }]} accessibilityLabel="Remove photo">
      ...
      <Animated.View entering={FadeInUp.delay(120)}>
        <Surface kind="raised" style={{ gap: space.lg }}>
          <Field placeholder={dog ? `At the park? Walk at 5? What should ${dog.name}'s people know?` : 'At the park? Who can walk at 5?'} value={caption} onChangeText={setCaption} multiline maxLength={400} style={{ minHeight: 90 }} error={error ?? undefined} />
          {dogs.length > 1 ? (
          ...
          <Button label={`Post to this circle  +${REWARDS.post.points}`} icon="send" onPress={post} loading={busy} disabled={!canPost || !circleId} size="lg" />
        </Surface>
      </Animated.View>
    </Screen>
  );
