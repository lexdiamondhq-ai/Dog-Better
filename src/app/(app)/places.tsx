  ...
  const search = useCallback(
    async (lat: number, lng: number) => {
      ...
      try {
        ...
        else if (list.length === 0) setError('No mapped dog spots within 5 km. Long-press the map to add one.');
      } catch (e) {
    ...
    <View style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      <View style={[styles.bottom, { paddingBottom: insets.bottom + space.md }]} pointerEvents="box-none">
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <PlaceCard place={item} km={item.km} pulse={pulses[item.id]} active={selected === item.id} onPress={() => router.push({ pathname: '/(app)/place/[id]', params: { id: item.id } })} />
          )}
        />
      </View>
...
function PlaceCard({ place, km, pulse, active, onPress }: { place: Place; km: number; pulse?: PulseSummary; active: boolean; onPress: () => void }) {
  const t = useTheme();
  const kind = place.kind as PlaceKind;
  return (
    <Tap onPress={onPress} haptic="medium" style={{ width: CARD_W }}>
        ...
        <View style={styles.cardRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text variant="caption" tone="secondary">
              {KIND_META[kind].label} - {formatDistance(km)}
            </Text>
          </View>
