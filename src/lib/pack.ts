...
export async function fetchFeed(userId: string, opts?: { authorId?: string }): Promise<FeedPost[]> {
...
export function useFeed(opts?: { authorId?: string }) {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setPosts(next);
    setLoading(false);
  }, [user, opts?.authorId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return { posts, loading, refreshing, refresh, like, reload: load };
}
